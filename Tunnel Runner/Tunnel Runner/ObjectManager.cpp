...
ObjectManager::ObjectManager(void):none(0),mud(1),magma(2),size(16),lava(3),flow(4),bedrock(5),gold(6),water(7),
									m_playerOne(new Player(Vector2f(Constants::SCREEN_WIDTH / 2, 0))), accel(0), spawnChance(600), lineDel(0),
		...
		for(int k = max(0, (int)((m_playerOne->getPosition().x - 32) / size)); k < min((int)map.at(i).size(), (int)((m_playerOne->getPosition().x + 64) / size)); k++)
		{
			...
			else if(map.at(i).at(k) == lava)
			{
				...
				if(m_playerOne->CheckBlock(lava, 2, lineDel, k, i))
				{
					...
					m_playerOne->updateHealth(-10 * timeElapsed);
				}
			}
		}
	}		
...
void ObjectManager::draw(SDL_Renderer* renderer)
{
	...
	pos.w = 32;
	pos.h = 32;
	SDL_RenderCopyEx(renderer, TextureManager::getManager()->miners->at(texNum + playerTex)->getTexture(), nullptr, &pos, 0, nullptr, (SDL_RendererFlip)(int)m_playerOne->getFlip());		
	
	TextureManager::getManager()->drawText(renderer, "Gold: " + std::to_string(m_playerOne->getScore()), 20, 20, TextureManager::positioning::left);
...
void ObjectManager::setUp()
{
	...
	m_playerOne->setPosition(Vector2f(Constants::SCREEN_WIDTH / 2, 0));
	accel = 0;
	m_playerOne->setScore(0);
	m_playerOne->setHealth(100);
	spawnChance = 600;
	lineDel = 0;
	timeTillMove = 0;
